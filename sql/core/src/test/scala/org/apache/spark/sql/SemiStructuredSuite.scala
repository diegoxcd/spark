...
class SemiStructuredSuite extends QueryTest with BeforeAndAfterAll {
  ...
  test("SemiStructured-2") {
    checkAnswer(
      ...
      Seq(Row(1,2.3,null, "option"),
        Row(2,3.0,2, null)
      )++(1 to 100).map(i => Row(1,i.toDouble,null,null))++(1 to 100).map(i => Row(2,i.toDouble,i+1,null))
    )
  }
  test( "123"){
    ...
    checkAnswer(
      sql("SELECT i+1 FROM nullableRepeatedData where i=2 "),
  ...
  test("SemiStructured-cached-case2") {
    ...
    checkAnswer(
      ...
      )++(1 to 100).map(i => Row(2,i.toDouble,i+1,null))
    )
  }
